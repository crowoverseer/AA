...
export const DaysTable: FC<DaysTableProps> = ({
	...
	const renderDay = (day: Day, dayIdx: number) => {
		...
		const inRange =
			...
			isWithinInterval(day.date, selection);
		...
		const transitLeft =
			firstDayOfMonth && inRange && selection && day.date > selection.start;
		const transitRight =
			lastDayOfMonth && inRange && selection && day.date < selection.end;
		...
		return (
			...
			<td
				...
				ref={(node) => {
					 ...
					 */
					if (
						startOfMonth(day.date).getTime() ===
						activeMonthRef.current.getTime()
					) {
			...
			>
				...
				>
					<span
						className={cn(styles.dayContent, {
							[styles.today]: isToday(day.date),
						})}
					>
						{day.date.getDate()}
	...
	return (
		<table
			className={cn(
				styles.daysTable,
				directionRef.current && styles[directionRef.current],
				{
