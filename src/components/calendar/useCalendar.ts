...
export function useCalendar({
	...
	const offDaysMap = useMemo(
		() => dateArrayToHashTable(offDays || []),
		[offDays],
	);

	const holidaysMap = useMemo(
		() => dateArrayToHashTable(holidays || []),
		[holidays],
	);

	const dayAddonsMap = useMemo(
		() => addonArrayToHashTable(dayAddons || []),
		[dayAddons],
	);
	...
	const focusDay = useCallback(
		(shift: DateShift) => {
			...
			if (focusedNode?.dataset.date) {
				...
				const newDate = modifyDateByShift(
					shift,
					focusedDate,
					minDate,
					maxDate,
				...
				if (
					newDate < focusedDate &&
					newDate.getMonth() !== focusedDate.getMonth()
				) {
					setPrevMonth();
					monthChanged = true;
				}
	...
	const focusYear = useCallback(
		(offset: number) => {
			...
			if (focusedNode?.dataset.date) {
				...
				if (
					newFocusedYear <= currentYear &&
					newFocusedYear > currentYear - years.length
				) {
	...
	const handleDayMouseEnter = useCallback(
		(event: MouseEvent<HTMLTableDataCellElement>) => {
			const { date } = (event.currentTarget as HTMLTableDataCellElement)
				.dataset;
