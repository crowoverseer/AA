...
export function generateMonths(
	year: Date,
	options: { minMonth?: Date; maxMonth?: Date; startMonth?: number },
) {
	return eachMonthOfInterval({
		start: startOfMonth(year.setMonth(options.startMonth || 0)),
		end: endOfYear(year),
	}).map((month) => buildMonth(month, options));
}
...
export function buildMonth(
	month: Date,
	options: { minMonth?: Date; maxMonth?: Date },
): Month {
...
export function dateInLimits(
	...
	return (
		date &&
		limitDate(date, minDate, maxDate).getTime() === new Date(date).getTime()
	);
}
